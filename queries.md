...
db.companies.find({ipo:{valuation_amount: {$gt: 100000000}}}, {name:1, _id:0, ipo:1})
...
db.companies.find({number_of_employees: {$lt: 1000}}).limit(10).sort({number_of_employees: 1})
...
db.companies.find({category_code:{$not: {$type: "string"}}})
...
<!--no encuentra nada pero si le cambias el año funciona-->
